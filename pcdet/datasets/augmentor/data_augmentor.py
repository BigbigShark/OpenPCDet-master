...
class DataAugmentor(object):
    ...
    def random_world_multi_frames_aug(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.random_multi_frames, config=config)
        points = data_dict['points']
        noise_translate_std = config['NOISE_TRANSLATE_STD']
        if noise_translate_std == 0:
            return data_dict
        points = getattr(augmentor_utils, 'random_multi_frames')(points, noise_translate_std)
